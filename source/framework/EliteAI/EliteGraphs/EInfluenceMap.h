...
namespace Elite
{
	...
	{
		//Calculate influence
		for(InfluenceNode* pNode : m_Nodes) //loop over nodes
		{
			for(auto pConnection : GetNodeConnections(pNode)) //loop over neighbours
			{
				float oldInfluence = pNode->GetInfluence();
				float newInfluence = oldInfluence * expf(-pConnection->GetCost() * m_Decay);
				
				m_InfluenceDoubleBuffer.push_back(Lerp(newInfluence, oldInfluence, m_Momentum));
			}
		}
