...
namespace Elite
{
	...
	public:
		...
		static std::vector<Elite::Vector2> OptimizePortals(const std::vector<Portal>& portals)
		{
			...
			Elite::Vector2 apex = portals[apexIndex].Line.p1;
			Elite::Vector2 rightLeg = portals[rightLegIndex].Line.p1 - apex;
			Elite::Vector2 leftLeg = portals[leftLegIndex].Line.p2 - apex;
			...
			for (unsigned int i = 1; i < static_cast<unsigned int>(portals.size()); ++i)
			{
				...
				if (Cross(rightLeg, newRightLeg) > 0) //going inwards
				{
					if (Cross(leftLeg, newRightLeg) >= 0) //legs cross
					{
						apex += leftLeg;
						apexIndex = leftLegIndex;

						unsigned int nextPortalIndex = apexIndex + 1;
						i = nextPortalIndex;
						leftLegIndex = nextPortalIndex;
						rightLegIndex = nextPortalIndex;

						//point not yet in path
						if (std::find(vPath.begin(), vPath.end(), apex) == vPath.end())
							vPath.push_back(apex);

						if (nextPortalIndex < static_cast<unsigned int>(portals.size()))
						{
				...
				if (Cross(newLeftLeg, leftLeg) > 0) //going inwards
				{
					if (Cross(newLeftLeg, rightLeg) >= 0) //legs cross
					{
						apex += rightLeg;
