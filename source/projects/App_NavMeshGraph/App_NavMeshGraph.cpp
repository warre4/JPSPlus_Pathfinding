...
void App_NavMeshGraph::Start()
{
	...
	m_pArriveBehavior->SetArriveRadius(1.0f);
...
std::vector<Elite::Vector2> App_NavMeshGraph::FindPath(Elite::Vector2 startPos, Elite::Vector2 endPos)
{
	...
	const Triangle* pStartTriangle{ m_pNavGraph->GetNavMeshPolygon()->GetTriangleFromPosition(startPos) };
	const Triangle* pEndTriangle{ m_pNavGraph->GetNavMeshPolygon()->GetTriangleFromPosition(endPos) };
	...
	//Copy the graph
	auto pTempGraph{ m_pNavGraph->Clone() }; //type = shared_ptr<IGraph>
	
	//Create extra node for the Start Node (Agent's position)
	int startIdx{ pTempGraph->GetNextFreeNodeIndex() };
	auto pStartNode = new NavGraphNode(startIdx, -1, startPos);
	pTempGraph->AddNode(pStartNode);

	for (int lineIdx : pStartTriangle->metaData.IndexLines)
	{
		if (lineIdx != -1)
		{
			int idx{ m_pNavGraph->GetNodeIdxFromLineIdx(lineIdx) };

			if (idx != invalid_node_index)
				pTempGraph->AddConnection(new GraphConnection2D(startIdx, idx));
		}
	}
	
	//Create extra node for the End Node
	int endIdx{ pTempGraph->GetNextFreeNodeIndex() };
	auto pEndNode = new NavGraphNode(endIdx, -1, endPos);
	pTempGraph->AddNode(pEndNode);
	...
	for (int lineIdx : pEndTriangle->metaData.IndexLines)
	{
		if (lineIdx != -1)
		{
			...
			if (idx != invalid_node_index)
				pTempGraph->AddConnection(new GraphConnection2D(endIdx, idx));
		}
	}
	
	//Run A star on new graph
	auto pathAStar{ AStar<NavGraphNode, GraphConnection2D>(pTempGraph.get(), HeuristicFunctions::Chebyshev).FindPath(pStartNode, pEndNode) };

	//DEBUG Rendering
	for (size_t i{ 1 }; i < pathAStar.size(); i++)
		m_DebugNodePositions.push_back(pathAStar[i]->GetPosition());

	//Extra: Run optimiser on new graph, Make sure the A star path is fine before uncommenting this!
	m_Portals = SSFA::FindPortals(pathAStar, m_pNavGraph->GetNavMeshPolygon());
